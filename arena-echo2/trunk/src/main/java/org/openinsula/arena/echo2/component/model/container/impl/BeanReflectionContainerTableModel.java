...
/**
 * This table model adds the ease of creating columns to define what
 * are they name and what properties will they show in each column.
 * @author realm
 *
 * @param <T>
 */
public class BeanReflectionContainerTableModel<T> extends BasicContainerTableModel<T> {
	...
	/**
	 * Searches for a table column with the name from the parameter
	...
	public Object getColumnValue(int columnIndex, T t) {
		...
		try {
			...
			logger.warn("There was an error getting the property '"+tableColumn.getProperty()+"' from a bean of type: "+t.getClass());
			e.printStackTrace();
		}
