...
public class LogradourosSeccionamentoLineFactory extends AbstractLineFactory {
	public LogradourosSeccionamentoLineFactory() {
		...
		addField(new NumeroTrechoField(11, "Número Inicial do Trecho"));
		addField(new NumeroTrechoField(11, "Número Final do Trecho"));
		addField(new StringField(1, "Identificação da Paridade"));
		addField(new NumericField(8, "Chave do Seccionamento no DNE"));
		addField(new StringField(74, "Separador"));
	}

	@Override
	public boolean doMatches(String s) {
		return "S".equals(getFieldString(s, 0));
	}
}
