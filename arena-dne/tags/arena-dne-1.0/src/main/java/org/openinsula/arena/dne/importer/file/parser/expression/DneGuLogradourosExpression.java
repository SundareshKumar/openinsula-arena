...
import org.openinsula.arena.dne.importer.file.line.LogradourosComplemento2LineFactory;
...
public class DneGuLogradourosExpression implements Expression {
	private Expression expression;

	public DneGuLogradourosExpression() {
		expression = new AndExpression(new LineExpression(new CabecalhoLineFactory()), new OrExpression(
				new LineExpression(new LogradourosDadosLineFactory()), new LineExpression(
						new LogradourosSeccionamentoLineFactory()), new LineExpression(
						new LogradourosNumeracaoLoteLineFactory()), new LineExpression(
						new LogradourosComplemento1LineFactory()), new LineExpression(
						new LogradourosComplemento2LineFactory())).oneOrMoreTimes(), new LineExpression(
				new TerminadorLineFactory()));
	}
